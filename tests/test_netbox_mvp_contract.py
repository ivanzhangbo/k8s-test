...
class NetBoxMvpContractTest(unittest.TestCase):
    ...
    def test_playbook_uses_rest_api_and_requests_config_context(self):
        ...
        self.assertIn('Authorization: "{{ netbox_auth_header }}"', playbook)
        self.assertNotIn('Authorization: "Bearer {{ netbox_token }}"', playbook)
        self.assertNotIn('Authorization: "Token {{ netbox_token }}"', playbook)
